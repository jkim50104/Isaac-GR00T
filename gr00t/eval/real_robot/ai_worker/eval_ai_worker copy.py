...
def eval(cfg: EvalConfig):
    ...
    try:
        while rclpy.ok():
            # Update ROS callbacks for subscriptions + allow publisher events
            ...
            # Query policy once, get a multi-step action chunk
            actions = policy.get_action(obs)

            # Stream up to action_horizon steps
            for i, action_blocks in enumerate(actions[: cfg.action_horizon]):
                tic = time.time()
                print(f"action[{i}]: {action_blocks}")

                # Send to robot
                sender.send_action_blocks(action_blocks)
